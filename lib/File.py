...
def DataFrame(data,menu):
    for value in data:
        result = list(value)
        if menu==2:
            df = pd.DataFrame([result], columns= ['Hostname','Type','Part','Description','Function','Serial Number'])
        elif menu==3:
            df = pd.DataFrame([result], columns= ['Hostname','Site','Memmory Used (MB)','Memmory Total (MB)','%','Status'])
        elif menu==4:
            df = pd.DataFrame([result], columns= ['Hostname','Site','CPU Utilization (%)','Status'])
        hdr = False if os.path.exists(f'./out/tmp-data.tmp') else True
        df.to_csv(f'./out/tmp-data.tmp', mode='a', header=hdr , index=False)

def ProcessExcel(nameFile):
    date_now   = datetime.now()
    time_stamp = date_now.strftime("%d-%m-%y %H_%M")
    path ='./out/'
