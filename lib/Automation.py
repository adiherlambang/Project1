...
def SetListCredential(list_credential):
    ...
    for list in range(list_credential):
        ...
        credential[list] = {'user':usr,'pass':pwd, 'sec':sec}
    return credential
    #### End Get Credential List from user ####

def ConnectDevice(ip,cmd,menu,data,**user):
    try:
            ...
            if result != False:
                if menu == 2:
                    ...
                    for dataDevice in result:
                        data.append([hostname[1],'',dataDevice['NAME'],dataDevice['DESCR'],'',dataDevice['SN']])                
                elif menu == 3:
                    ...
                    for dataDevice in result:
                        ...
                        if percent <= 20:
                            ...
                            status = 'CRITICAL'
                        data.append([hostname[1],'',int(memUsed),int(memTotal),int(percent),status])
                elif menu == 4:
                    ...
                    for dataDevice in result:
                        ...
                        if cpu <= 20:
                            ...
                            status = 'CRITICAL'
                        data.append([hostname[1],'',cpu,status])
